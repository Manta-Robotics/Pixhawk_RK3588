...
def build_capture_command() -> list[str]:
    command = [
        ...
        '-hide_banner',
        '-loglevel', 'error',
        '-fflags', '+nobuffer+discardcorrupt' if LOW_LATENCY else '+genpts+discardcorrupt',
        '-flags', 'low_delay',
        '-thread_queue_size', '32' if LOW_LATENCY else '512',
    ]
    command.extend([
        '-rtsp_transport', str(VIDEO.get('rtsp_transport', 'tcp')),
        '-analyzeduration', '0',
        '-probesize', '32768',
    ])
    command.extend([
            ...
            while True:
                ...
                if not ready:
                    if time.time() - last_frame_at > FRAME_STALL_SECONDS:
                        raise RuntimeError('gimbal rtsp stream stalled; reconnecting')
                    continue
    ...
    def do_GET(self):
        ...
        if path in ('/', '/snapshot.jpg', '/frame.jpg'):
            with FRAME_LOCK:
                ...
                err = FRAME_CACHE['error']
            if not frame:
                self._send_json({'ok': False, 'message': err or 'Gimbal stream not ready yet.', 'inputUrl': INPUT_URL}, status=503)
                return
